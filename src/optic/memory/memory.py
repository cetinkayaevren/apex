...
class Memory(Module):
    def __init__(self, memory_size = 100, prompt_size = 6, temperature=0.1, cont_temp=0.07,  device = "cuda:0", lambda_factor=0.4):
        ...
        self.latent_feature = 24
        self.contrastive_out= 36
        self.out_features = self.latent_feature
        ...
        self.decoder_model = Sequential(
            Linear(in_features= self.out_features, out_features=36),
            ReLU(),
        ...
        self.contrastive_mlp = Sequential(
            Linear(in_features= self.latent_feature, out_features=24),
            ReLU(),
            Linear(in_features= 24, out_features=36),
            ReLU(),
            Linear(in_features= 36, out_features=self.contrastive_out),
        )

        self.memory_shape = [self.memory_size, self.out_features]
        self.memory_w = nn.init.normal_(torch.empty(self.memory_shape), mean=0.0, std=1.0) #nn.init.orthogonal_(torch.empty(self.memory_shape), gain=1.0)    #nn.init.normal_(torch.empty(self.memory_shape), mean=0.0, std=1.0) #Maybe change the initialization!!
    ...
    def forward(self, input_img, domain_gt):
        ...
        frequency_feature = motion_encoder(flattened_img) #self.encoder_model(flattened_img)   

        #Sent to Contrastive MLP for computing Contrastive Loss
        contrastive_feature = self.contrastive_mlp(frequency_feature.detach())
        contrastive_feature = F.normalize(contrastive_feature, dim=1)
        contrastive_feature = contrastive_feature.reshape(-1, 1, self.contrastive_out)
