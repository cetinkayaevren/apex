...
from loss.bce_dice_loss import BCEDiceLoss
from models.UNet.UNet import UNet
from configs.load_config_swin import swin_config
import logging
#from dataloader.dataloader_pranet import PromptingDataPraNet
...
source_domain_names = ["CVC-ClinicDB_0", "Kvasir-SEG_3"]
target_domain_names = ["CVC-ClinicDB_0", "Kvasir-SEG_3", "ETIS-LaribPolypDB", "CVC-ColonDB"]

print(f"{selected_model} is selected.")

#Create Logging Config
logging.basicConfig(level=logging.INFO, filename=f"polyp_memory_training_{selected_model}_mem_size_{MEM_SIZE}.log", filemode="w",
...
TARGET_IMAGE_DOMAIN_3 = DATASET_ROOT / target_domain_names[2] / "test" / "image"
TARGET_MASK_DOMAIN_3 = DATASET_ROOT / target_domain_names[2] / "test" / "mask"

TARGET_IMAGE_DOMAIN_4 = DATASET_ROOT / target_domain_names[3] / "image"
TARGET_MASK_DOMAIN_4 = DATASET_ROOT / target_domain_names[3] / "mask"

MEMORY_WEIGHTS_DIR = OUTPUT_DIR / "memory_weights"
MEMORY_BEST_PATH = MEMORY_WEIGHTS_DIR / f"prompt_memory_polyp_{selected_model}_prompt_{PROMPT_DIM}_epoch_{EPOCHS}_lr_{LEARNING_RATE}_add_{ADDRESSING_TYPE}_mem_{MEM_SIZE}_temp_{TEMPERATURE}_best.pth"
MEMORY_LAST_PATH = MEMORY_WEIGHTS_DIR / f"prompt_memory_polyp_{selected_model}_prompt_{PROMPT_DIM}_epoch_{EPOCHS}_lr_{LEARNING_RATE}_add_{ADDRESSING_TYPE}_mem_{MEM_SIZE}_temp_{TEMPERATURE}_last.pth"
...
train_indices, val_indices= random_split(range(len(dataset_image_prompt)), [0.9, 0.1], generator=generator1)   #[1878, 200]

train_dataset = PromptingDataMemorySubset(dataset_image_prompt, train_indices, input_size=INPUT_SIZE, augment=False)
val_dataset = PromptingDataMemorySubset(dataset_image_prompt, val_indices, input_size=INPUT_SIZE, augment=False)
...
#Loss loading
seg_loss = BCEDiceLoss(weight_BCE = 0.5, weight_Dice = 0.5)
...
for e in tqdm(range(EPOCHS)):
    ...
    for (i, (images, masks, domain_gt)) in enumerate(trainLoader):
        ...
        (images, masks, domain_gt) = (images.to(DEVICE), masks.to(DEVICE), domain_gt.to(DEVICE))

        # perform a forward pass and calculate the training loss
        prompted_images, addressing_vec, pred_prompt, contrastive_loss = prompt_memory(images, domain_gt)

        pred = torch.sigmoid(model(prompted_images))
        
        # Compute the both segmentation and contrastive loss
    ...
    with torch.no_grad():
        ...
        for (images, masks, domain_gt) in valLoader:
            ...
            _loss_val = seg_loss(pred, masks)

            total_val_dice += dice(pred, masks.long().to(DEVICE)) / valSteps
...
#----------------- Testing Phase ---------------------
    ...
    with torch.no_grad():
        ...
        for (i, (images, masks, domain_gt,_)) in enumerate(testLoader):
            images, labels, domain_gt = images.to(DEVICE), masks.to(DEVICE), domain_gt.to(DEVICE)    #.long()
            ...
            # Accumulate metrics
            total_dice += dice(preds, labels.long().to(DEVICE)) / testSteps
            total_miou += miou(preds.long(), labels.long().to(DEVICE)) / testSteps
...
#----------------- Testing Phase 2---------------------
    ...
    with torch.no_grad():
        ...
        for (i, (images, masks, domain_gt,_)) in enumerate(testLoader2):
            images, labels, domain_gt = images.to(DEVICE), masks.to(DEVICE), domain_gt.to(DEVICE)    #.long()
            ...
            preds = torch.round(preds)
            # Accumulate metrics
            total_dice2 += dice(preds, labels.long().to(DEVICE)) / testSteps2
            total_miou2 += miou(preds.long(), labels.long().to(DEVICE)) / testSteps2
...
#----------------- Testing Phase 3---------------------
    ...
    with torch.no_grad():
        ...
        for (i, (images, masks, domain_gt,_)) in enumerate(testLoader3):
            images, labels, domain_gt = images.to(DEVICE), masks.to(DEVICE), domain_gt.to(DEVICE)    #.long()
            ...
            preds = torch.round(preds)
            # Accumulate metrics
            total_dice3 += dice(preds, labels.long().to(DEVICE)) / testSteps3
            total_miou3 += miou(preds.long(), labels.long().to(DEVICE)) / testSteps3
...
#----------------- Testing Phase 4---------------------
    ...
    with torch.no_grad():
        ...
        for (i, (images, masks, domain_gt,_)) in enumerate(testLoader4):
            images, labels, domain_gt = images.to(DEVICE), masks.to(DEVICE), domain_gt.to(DEVICE)    #.long()
            ...
            preds = torch.round(preds)
            # Accumulate metrics
            total_dice4 += dice(preds, labels.long().to(DEVICE)) / testSteps4
            total_miou4 += miou(preds.long(), labels.long().to(DEVICE)) / testSteps4
    ...
    print("""Train loss: {:.6f}, 
        ...
        avgTrainLoss, 
        avgContLoss, 
        total_val_dice.cpu().detach().numpy(),
        average_total_dice,
        total_dice.cpu().detach().numpy(), 
        total_miou.cpu().detach().numpy(),
        total_dice2.cpu().detach().numpy(), 
        total_miou2.cpu().detach().numpy(),
        total_dice3.cpu().detach().numpy(),
        total_miou3.cpu().detach().numpy(),
        total_dice4.cpu().detach().numpy(),
        total_miou4.cpu().detach().numpy(),
        current_lr))
    
    logging.info(f"[INFO] EPOCH: {e + 1}/{EPOCHS}")
    logging.info(f"Train loss: {avgTrainLoss}, Cont Loss: {avgContLoss}, Validation loss: {avgValLoss}, Avg. Total DICE: {average_total_dice}, CVC-Clin DICE: {total_dice.cpu().detach().numpy()}, CVC-Clin mIoU: {total_miou.cpu().detach().numpy(),}, Kvasir DICE: {total_dice2.cpu().detach().numpy()}, Kvasir mIoU: {total_miou2.cpu().detach().numpy()}, ETIS DICE: {total_dice3.cpu().detach().numpy()}, ETIS mIoU: {total_miou3.cpu().detach().numpy()}, CVC DICE: {total_dice4.cpu().detach().numpy()}, CVC mIoU: {total_miou4.cpu().detach().numpy()}")
